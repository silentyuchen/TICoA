...
class ConvDecoder(nn.Module):
    ...
    def forward(self, x, *args):
        ...
        self.save_feat_png(out0, "out0")
        self.save_feat_png(out1, "out1")
        self.save_feat_png(out2, "out2")
        self.save_feat_png(out3, "out3")
        out = self.mixer3(out0)
        self.save_feat_png(out, "mixer3_out")
        out = F.interpolate(out, scale_factor=2, mode='bilinear', align_corners=True)
        self.save_feat_png(out, "mixer3_out_interpolate")
        out = self.mixer2(torch.cat([out, out1], dim=1))
        self.save_feat_png(out, "mixer2_out")
        out = F.interpolate(out, scale_factor=2, mode='bilinear', align_corners=True)
        self.save_feat_png(out, "mixer2_out_interpolate")
        out = self.mixer1(torch.cat([out, out2], dim=1))
        self.save_feat_png(out, "mixer1_out")
        out = F.interpolate(out, scale_factor=2, mode='bilinear', align_corners=True)

        out = self.neck(torch.cat([out, out3], dim=1))
        self.save_feat_png(out, "neck_out")
        return out
    
    def save_feat_png(self, feat, name, colormap='Blues', channel=None):
        """
        保存特征图为 PNG（稳健版）
        feat: Tensor
        name: 文件名
        ...
        if x.dim() == 4:
            # [B,C,H,W] or [B,H,W,C]
            if x.shape[1] <= 4:  # [B,H,W,C]
                x = x.permute(0, 3, 1, 2)
            x = x.mean(1)[0]

        elif x.dim() == 3:
            # [C,H,W] or [H,W,C]
            if x.shape[0] <= 4:  # [H,W,C]
                x = x.permute(2, 0, 1)
